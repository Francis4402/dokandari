...
const Customers = ({ customers: initialCustomers }: PageProps) => {
  ...
  const getRoleColor = (role: Customer['role']) => {
    const colors: Record<Customer['role'], string> = {
  ...
  const getRoleLabel = (role: Customer['role']) => {
    const labels: Record<Customer['role'], string> = {
                              ...
                              <div className="flex items-start justify-between mb-2">
                                <div>
                                  ...
                                  <div className="flex items-center mt-1 space-x-2">
                                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${getRoleColor(customer.role)}`}>
                                      <span className="ml-1">{getRoleLabel(customer.role)}</span>
                                    </span>
                ...
                <div className="bg-white rounded-xl shadow-lg p-6">
                  ...
                  <div className="text-center mb-6">
                    <div className="w-24 h-24 rounded-full overflow-hidden border-4 border-white shadow-lg mx-auto mb-4">
                      <img
                        src={selectedCustomer.images || `https://ui-avatars.com/api/?name=${encodeURIComponent(selectedCustomer.name)}&background=random`}
                    ...
                    <div className="flex items-center justify-center mt-2 space-x-2">
                      <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getRoleColor(selectedCustomer.role)}`}>
                        <span className="ml-2">{getRoleLabel(selectedCustomer.role)}</span>
                      </span>
